...
DEXKIT_JAR="libs/dexkit.jar"
KOTLIN_JAR="libs/kotlin-stdlib.jar"
FLATBUF_JAR="libs/flatbuffers-java.jar"
DEXKIT_SO="libs/jni/arm64-v8a/libdexkit.so"
...
if ! javac -source 8 -target 8 -cp "$ANDROID_JAR:$API_JAR:$DEXKIT_JAR:$KOTLIN_JAR" -d $OUT/classes @$OUT/sources.txt 2> $OUT/javac.err; then
  cat $OUT/javac.err
  exit 1
fi
grep -v "warning:" $OUT/javac.err || true

echo "[2/6] d8 (module classes + DexKit runtime -> dex)"
MODCLASSES=$(find $OUT/classes/com/dsmod -name "*.class")
$D8 --min-api 24 --output $OUT/dex \
    $MODCLASSES "$DEXKIT_JAR" "$KOTLIN_JAR" "$FLATBUF_JAR" \
    --lib "$ANDROID_JAR"
...
cp "$DEXKIT_SO" $OUT/apklib/lib/arm64-v8a/libdexkit.so
( cd $OUT/apklib && zip -q -0 -X ../unsigned.apk lib/arm64-v8a/libdexkit.so )
...
echo "[5/6] zipalign"
$ZIPALIGN -f -P 16 4 $OUT/unsigned.apk $OUT/aligned.apk
